    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <Form action={routes.store().url} method="post" className="mx-auto w-full max-w-xl space-y-4" onSuccess={() => add({ title: 'تم الحفظ', description: 'تم إنشاء المخزن بنجاح' })} onError={() => add({ title: 'خطأ', description: 'تعذر إنشاء المخزن', variant: 'destructive' })}>
                    {({ processing, errors }) => (
                        <>
